...
# Context Dependent Spelling Corrector
...
def nextwd(ws):
    ...
    for w in ws:
        if w not in tmp_dict:
            tmp_dict[w] = []
        tmp_dict[w].append(next_w)
        next_w = w
    return tmp_dict

def bestnext(prev, lst, next_dict):
    #order = {}
    best = lst[0]
    try:
        cter = Counter(next_dict[prev])
    except KeyError:
        return lst[0]
    for nx in lst:
        if cter[nx] > cter[best]:
            best = nx
    return best
...
def doyoumean(str):
    ...
    for i in range(1, len(lst)):
        wd = lst[i]
        if 0 == i:
            # prev = bestnext(lst[1], correct(wd),PREV_DICT)
            pass
        else:
            prev = bestnext(prev, correct(wd), NEXT_DICT)
            restr += " "
            restr += prev
    return lst[0] + " " + restr

# return bestnext(second, correct(lst[0]),NEXT_DICT) + " " + restr
...
file_read = open('./big.txt', "r",encoding='iso-8859-1')
NWORDS = train(words(file_read.read()))
file_read2 = open('./big.txt', "r",encoding='iso-8859-1')
all_words = words(file_read2.read())
NEXT_DICT = nextwd(all_words)
all_words.reverse()
PREV_DICT = nextwd(all_words)
#print(PREV_DICT)
#print(NEXT_DICT['I'])
...
def correct(word):
    ...
    #for i in candidates:
    #	print(i, NWORDS[i])
    return sorted(candidates, key=lambda w: NWORDS[w], reverse=True)
...
if __name__ == '__main__':
    #lst = ['biulding', 'reequired', 'nessisitates', 'studens']
    #for i in lst:
        #print("{},{}\n".format(i, correct(i)))
        #print(spelltest(tests1))

    print(correct('shoer'))
    #print(bestnext('fish',correct('tink')))

    #test_str = "buiding', 'reequired', 'nessisitates','studens'"
    #test_str = "fish tink"
    #test_str = "sea shoer"

    #query = input("Please enter a query:")
    print(doyoumean(query))
